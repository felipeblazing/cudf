...
gdf_size_type make_aligned_size(gdf_size_type size)
{
    ...
    return cudf::util::round_up_to_power_of_2<gdf_size_type>(size, alignment_quantum);
}
    ...
    if (cudf::is_nullable(*column)) {
        ...
        if (output->size > 0) {
            auto pack_grid_config = cudf::util::form_naive_1d_grid(
                cudf::util::round_up_to_power_of_2<gdf_size_type>(output->size, bits_per_container),
                threads_per_block,
                bits_per_container);
